...
chats = {}
plugins = {}
helper_id = 000000--Put Your Helper Bot ID Here
...
function create_config( )
  -- A simple config with basic plugins and ourselves as privileged user
    ...
    sudo_users = {377450049,000000,000000,000000,000000,000000},
    admins = {},
    disabled_channels = {},
...
function pre_processb_msg(msg)
  for name,plugin in pairs(plugins) do
    if plugin.pre_processb and msg then
		print('Preprocessb:', name)
		pre_msgb = plugin.pre_processb(msg)
    end
  end
  return pre_msgb
end
function pre_processw_msg(msg)
  for name,plugin in pairs(plugins) do
    if plugin.pre_processw and msg then
		print('Preprocessw:', name)
		pre_msgw = plugin.pre_processw(msg)
    end
  end
  return pre_msgw
end
function pre_processt_msg(msg)
  for name,plugin in pairs(plugins) do
    if plugin.pre_processt and msg then
		print('Preprocess:', name)
		pre_msg = plugin.pre_processt(msg)
    end
  end
  return pre_msg
end
 ...
 function var_cb(msg, data)
  -------------Get Var-------------
	...
	tdcli_function({ ID = 'GetMe'}, botifo_cb, {chat_id=msg.chat_id_})
     ...
     False = false
     pre_process_msg(msg)
	 pre_processt_msg(msg)
	 pre_processb_msg(msg)
	 pre_processw_msg(msg)
		match_plugins(msg)
	end
